...
import CompanyUser from './CompanyUserModel.js';
...
export const getSupervisors = async (req, res) => {
  try {
    ...
    // Get all userIds from companyUsers
    const userIds = companyUsers.map(cu => cu.userId);

    // Fetch employee info (for fullName) for these userIds
    const employees = await Employee.find({ userId: { $in: userIds } })
      .select('userId fullName email phone companyId')
      .lean();

    // Map userId to employee info for easy lookup
    const employeeMap = {};
    ...
    // Format response
    const supervisors = companyUsers.map(cu => {
      const emp = employeeMap[cu.userId] || {};
      return {
        id: cu.userId,
