...
const createTaskAssignmentsForCurrentUser = async () => {
  try {
    console.log('📝 Creating task assignments for current authenticated user...\n');

    // Find the employee with ID 106 (from the JWT debug)
    const employee = await Employee.findOne({ id: 106 });
    if (!employee) {
      console.log('❌ Employee ID 106 not found');
...
const main = async () => {
  await connectDB();
  await createTaskAssignmentsForCurrentUser();
