import mongoose from 'mongoose';
import bcrypt from 'bcryptjs';
import dotenv from 'dotenv';

dotenv.config();

const User = mongoose.model('User', new mongoose.Schema({}, { strict: false, collection: 'users' }));
const Employee = mongoose.model('Employee', new mongoose.Schema({}, { strict: false, collection: 'employees' }));
const Driver = mongoose.model('Driver', new mongoose.Schema({}, { strict: false, collection: 'drivers' }));
const Project = mongoose.model('Project', new mongoose.Schema({}, { strict: false, collection: 'projects' }));
const Vehicle = mongoose.model('Vehicle', new mongoose.Schema({}, { strict: false, collection: 'vehicles' }));

async function fixAndCreateDriver() {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('✅ Connected to MongoDB\n');

    // Driver credentials
    const driverEmail = 'driver@construction.com';
    const driverPassword = 'Driver@123';
    const driverName = 'Rajesh Kumar';
    
    console.log('📋 Creating driver account with:');
    console.log('   Email:', driverEmail);
    console.log('   Password:', driverPassword);
    console.log('   Name:', driverName);
    console.log('');

    // Step 1: Fix null ID employees
    console.log('🔧 Fixing employees with null ID...');
    const nullIdEmployees = await Employee.find({ id: null });
    console.log(`   Found ${nullIdEmployees.length} employees with null ID`);
    
    if (nullIdEmployees.length > 0) {
      const maxEmployee = await Employee.findOne({ id: { $type: 'number' } }).sort({ id: -1 });
      let nextId = maxEmployee && typeof maxEmployee.id === 'number' ? maxEmployee.id + 1 : 210;
      
      for (const emp of nullIdEmployees) {
        await Employee.updateOne({ _id: emp._id }, { $set: { id: Number(nextId) } });
        console.log(`   ✅ Updated employee ${emp._id} with ID: ${nextId}`);
        nextId++;
      }
    }

    // Step 1b: Fix null ID users
    console.log('\n🔧 Fixing users with null ID...');
    const nullIdUsers = await User.find({ id: null });
    console.log(`   Found ${nullIdUsers.length} users with null ID`);
    
    if (nullIdUsers.length > 0) {
      const maxUser = await User.findOne({ id: { $type: 'number' } }).sort({ id: -1 });
      let nextUserId = maxUser && typeof maxUser.id === 'number' ? maxUser.id + 1 : 110;
      
      for (const usr of nullIdUsers) {
        await User.updateOne({ _id: usr._id }, { $set: { id: Number(nextUserId) } });
        console.log(`   ✅ Updated user ${usr._id} with ID: ${nextUserId}`);
        nextUserId++;
      }
    }

    // Step 2: Check if driver already exists
    const existingUser = await User.findOne({ email: driverEmail });
    if (existingUser) {
      console.log('\n⚠️  Driver user already exists, deleting old records...');
      await User.deleteOne({ email: driverEmail });
      await Employee.deleteOne({ email: driverEmail });
      await Driver.deleteOne({ email: driverEmail });
    }

    // Step 3: Get project and vehicle
    const project = await Project.findOne();
    if (!project) {
      console.log('❌ No projects found. Please create a project first.');
      return;
    }
    console.log('\n✅ Found project:', project.name, '(ID:', project._id, ')');

    let vehicle = await Vehicle.findOne();
    if (!vehicle) {
      console.log('📦 Creating sample vehicle...');
      vehicle = await Vehicle.create({
        vehicleNumber: 'KA-01-AB-1234',
        vehicleType: 'Bus',
        capacity: 30,
        status: 'active',
        company: project.company
      });
      console.log('✅ Vehicle created:', vehicle.vehicleNumber);
    } else {
      console.log('✅ Found vehicle:', vehicle.vehicleNumber);
    }

    // Step 4: Generate new employee ID
    const maxEmp = await Employee.findOne({ id: { $type: 'number' } }).sort({ id: -1 });
    const newEmployeeId = maxEmp && typeof maxEmp.id === 'number' ? maxEmp.id + 1 : 210;
    console.log('\n📝 New Employee ID will be:', newEmployeeId);

    // Step 4b: Generate new user ID
    const maxUsr = await User.findOne({ id: { $type: 'number' } }).sort({ id: -1 });
    const newUserId = maxUsr && typeof maxUsr.id === 'number' ? maxUsr.id + 1 : 110;
    console.log('📝 New User ID will be:', newUserId);

    // Step 5: Hash password
    const hashedPassword = await bcrypt.hash(driverPassword, 10);

    // Step 6: Create Employee record
    console.log('\n📝 Creating Employee record...');
    const employee = await Employee.create({
      id: newEmployeeId,
      name: driverName,
      email: driverEmail,
      phone: '+91-9876543210',
      role: 'driver',
      company: project.company,
      projects: [project._id],
      status: 'active',
      dateOfJoining: new Date(),
      address: 'Bangalore, Karnataka',
      emergencyContact: {
        name: 'Sunita Kumar',
        phone: '+91-9876543211',
        relationship: 'Spouse'
      }
    });
    console.log('✅ Employee created with ID:', employee._id);

    // Step 7: Create User record
    console.log('\n👤 Creating User record...');
    const user = await User.create({
      id: newUserId,
      email: driverEmail,
      password: hashedPassword,
      name: driverName,
      role: 'driver',
      employeeId: employee._id,
      currentProject: project._id,
      permissions: {
        canViewTrips: true,
        canManageTrips: true,
        canViewVehicles: true,
        canReportIncidents: true
      }
    });
    console.log('✅ User created with ID:', user._id);

    // Step 8: Create Driver record
    console.log('\n🚗 Creating Driver record...');
    const driver = await Driver.create({
      employeeId: employee._id,
      userId: user._id,
      name: driverName,
      email: driverEmail,
      phone: '+91-9876543210',
      licenseNumber: 'KA-1234567890',
      licenseType: 'Commercial',
      licenseExpiry: new Date('2026-12-31'),
      vehicleAssigned: vehicle._id,
      vehicleNumber: vehicle.vehicleNumber,
      status: 'active',
      company: project.company,
      projects: [project._id],
      experience: 8,
      rating: 4.5,
      totalTrips: 0,
      address: 'Bangalore, Karnataka',
      emergencyContact: {
        name: 'Sunita Kumar',
        phone: '+91-9876543211',
        relationship: 'Spouse'
      }
    });
    console.log('✅ Driver created with ID:', driver._id);

    // Step 9: Update vehicle
    await Vehicle.updateOne(
      { _id: vehicle._id },
      { 
        $set: { 
          assignedDriver: driver._id,
          driverName: driverName
        } 
      }
    );
    console.log('✅ Vehicle updated with driver assignment');

    // Step 10: Verification
    console.log('\n🔍 VERIFICATION:');
    const verifyUser = await User.findOne({ email: driverEmail });
    const verifyEmployee = await Employee.findOne({ email: driverEmail });
    const verifyDriver = await Driver.findOne({ email: driverEmail });
    
    console.log('   User exists:', !!verifyUser ? '✅' : '❌');
    console.log('   Employee exists:', !!verifyEmployee ? '✅' : '❌');
    console.log('   Driver exists:', !!verifyDriver ? '✅' : '❌');

    console.log('\n✅ DRIVER ACCOUNT CREATED SUCCESSFULLY!');
    console.log('\n📱 LOGIN CREDENTIALS:');
    console.log('   Email:', driverEmail);
    console.log('   Password:', driverPassword);
    console.log('\n📊 ACCOUNT DETAILS:');
    console.log('   Name:', driverName);
    console.log('   Role: Driver');
    console.log('   License:', driver.licenseNumber);
    console.log('   Vehicle:', vehicle.vehicleNumber);
    console.log('   Project:', project.name);
    console.log('   Employee ID:', employee.id);
    console.log('   Employee _id:', employee._id);
    console.log('   User ID:', user._id);
    console.log('   Driver ID:', driver._id);

  } catch (error) {
    console.error('❌ Error:', error.message);
    console.error(error);
  } finally {
    await mongoose.connection.close();
    console.log('\n🔌 Database connection closed');
  }
}

fixAndCreateDriver();
